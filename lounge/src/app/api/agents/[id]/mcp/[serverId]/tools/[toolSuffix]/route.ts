/**
 * TASK-095 v3.1 Batch 3: MCP tool toggle.
 *
 * PATCH /api/agents/:id/mcp/:serverId/tools/:toolSuffix — Toggle tool enabled/disabled
 */
...
export async function PATCH(
  request: Request,
  { params }: { params: Promise<{ id: string; serverId: string; toolSuffix: string }> }
) {
  ...
  const { id, serverId, toolSuffix } = await params;
