/**
 * GET /api/agents/:id/pool — Day memory pool proxy.
 * TASK-108: Proxies to engine's /api/dashboard/pool endpoint.
 */
...
export async function GET(
  ...
  const result = await dashboardGet(agent.port, keys[0].key, 'pool');
  if (!result) {
