/**
 * GET /api/agents/:id/threads — Active threads proxy.
 * TASK-108: Proxies to engine's /api/dashboard/threads endpoint.
 */
...
export async function GET(
  ...
  const result = await dashboardGet(agent.port, keys[0].key, 'threads');
  if (!result) {
