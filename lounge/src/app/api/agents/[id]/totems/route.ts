/**
 * GET /api/agents/:id/totems — Totems proxy.
 * TASK-108: Proxies to engine's /api/dashboard/totems endpoint.
 */
...
export async function GET(
  ...
  const result = await dashboardGet(agent.port, keys[0].key, 'totems');
  if (!result) {
