/**
 * GET /api/agents/:id/summaries — Daily summaries proxy.
 * TASK-108: Proxies to engine's /api/dashboard/daily-summaries endpoint.
 */
...
export async function GET(
  ...
  const result = await dashboardGet(agent.port, keys[0].key, 'daily-summaries');
  if (!result) {
