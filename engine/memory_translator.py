...
Converts drive values, mood states, and internal pipeline data into natural
language suitable for conscious memory (MD files).  No raw numbers, no
percentages, no drive names.

The single-word helpers (mood_word, drive_level, energy_word) are shared with
prompt/self_context.py to keep one source of truth.  The richer sentence
generators (translate_mood, translate_drive, etc.) are used exclusively by
the memory writer layer.
"""

import re


# ── Single-word helpers (shared with self_context.py) ──
...
def translate_drives_summary(drives) -> str:
    """Convert a full drives state to a paragraph of feeling.

    Args:
        drives: object with mood_valence, mood_arousal, social_hunger,
                diversive_curiosity, expression_need, rest_need, energy attrs.

    Returns natural language paragraph.  NO numbers.
    """
    parts = []

    mood_sent = translate_mood(drives.mood_valence, drives.mood_arousal)
    if mood_sent:
    ...
    energy_sent = translate_energy(getattr(drives, 'energy', 0.5))
    if energy_sent:
...
def translate_internal_conflict(conflicts: list[str]) -> str:
    ...
    Args:
        conflicts: list of raw conflict strings from the metacognitive monitor,
                   e.g. ["Used exclamation mark without being surprised"]

    Returns a natural first-person observation.
    """
    if not conflicts:
        return ''

    # Map common machine-readable patterns to felt descriptions
    _CONFLICT_MAP = [
    ...
    for conflict in conflicts[:3]:  # cap at 3
        matched = False
        for pattern, felt in _CONFLICT_MAP:
            if re.search(pattern, conflict, re.IGNORECASE):
                felt_parts.append(felt)
                matched = True
                break
        if not matched:
            # Generic fallback — still first-person, still felt
...
def scrub_numbers(text: str) -> str:
    """Strip stray numbers/percentages from text.

    Safety net called on every string before writing to MD files.
