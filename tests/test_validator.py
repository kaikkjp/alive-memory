...
from models.pipeline import (
    CortexOutput, ValidatorState, ValidatedOutput, ActionRequest,
)
...
class TestSchemaDefaults:
    ...
    def test_missing_dialogue_gets_silence(self):
        result = validate(CortexOutput(), ValidatorState(cycle_type='idle'))
...
class TestEngagementGate:
    ...
    def test_journal_blocked_during_engagement(self, sample_cortex_output, engaged_state):
        sample_cortex_output.actions = [ActionRequest(type='write_journal')]
        ...
        assert result.journal_deferred is True
    ...
    def test_end_engagement_blocked_before_turn_3(self, sample_cortex_output, engaged_state):
        ...
        sample_cortex_output.actions = [ActionRequest(type='end_engagement')]
        result = validate(sample_cortex_output, engaged_state)
        assert len(result.approved_actions) == 0
        assert 'too early' in result.dropped_actions[0].reason
    ...
    def test_close_shop_allowed_when_exhausted(self, sample_cortex_output, engaged_state):
        ...
        sample_cortex_output.actions = [ActionRequest(type='close_shop')]
        result = validate(sample_cortex_output, engaged_state)
        assert len(result.approved_actions) == 1
...
class TestEntropyCheck:
    ...
    def test_no_warning_on_first_use(self):
        output = ValidatedOutput(dialogue='Hello there, welcome.')
        result = entropy_check(output)
        assert result.entropy_warning is None

    def test_warns_on_repeated_opening(self):
        opening = "The rain falls softly outside."
        for _ in range(6):
            entropy_check(ValidatedOutput(dialogue=opening))
        result = entropy_check(ValidatedOutput(dialogue=opening))
        assert result.entropy_warning is not None

    def test_silence_not_tracked(self):
        for _ in range(10):
            result = entropy_check(ValidatedOutput(dialogue='...'))
        assert result.entropy_warning is None
