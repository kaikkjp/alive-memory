...
Part of alive_cognition (moved from alive_memory.identity.evolution).
...
from alive_cognition.identity.drift import DriftReport, DriftResult
...
class GuardRailConfig:
    ...
    protected_traits: dict[str, tuple[float, float]] = field(default_factory=dict)
...
class IdentityEvolution:
    ...
    def __init__(
        ...
        on_decision: Callable[[EvolutionDecision], Awaitable[None]] | None = None,
    ):
    ...
    async def evaluate(self, report: DriftReport | DriftResult) -> EvolutionDecision:
    ...
    async def _evaluate_drift_report(self, report: DriftReport) -> EvolutionDecision:
    ...
    async def _evaluate_drift_result(self, result: DriftResult) -> EvolutionDecision:
    ...
    async def apply(self, decision: EvolutionDecision) -> None:
        ...
        if decision.action == "correct" and decision.correction_value is not None:
            from alive_cognition.identity.self_model import update_traits

            await update_traits(self._storage, {decision.trait: decision.correction_value})
