...
async def log_cost(
    ...
    try:
        ...
        # This runs as a fire-and-forget asyncio.create_task — unhandled
        # exceptions here silently accumulate as coroutine objects and can
        # cause memory pressure. Common cause: DB lock contention when
        # parallel sims write to the production DB.
        print(f"[CostLog] failed to log cost ({type(exc).__name__}: {exc})")
